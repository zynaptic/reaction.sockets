...
/**
 * Implements a data source for socket tests. The data source may be a simple
 * counting sequence or a pseudo-random sequence generated by the standard Java
 * {@link Random} library class.
 * 
 * @author Chris Holgate
 */
public class SocketTestDataSource implements Deferrable<ByteBuffer, Void> {
  ...
  /**
   * Provides the public constructor for the data source.
   * 
   ...
   *   be used for selecting buffer sizes and transmission intervals. A null
   ...
   *   generated by the test data source.
   */
  public SocketTestDataSource(Reactor reactor, Logger logger, SocketService socketService, SocketHandle socketHandle,
    ...
    if (dataSeed == null) {
      ...
      this.dataSource = new Random(dataSeed);
    }
    if (randomSeed == null) {
  ...
  /**
   * Provides the call point for starting the data generation process. This should
   * be called exactly once for each data source instance.
   */
  public synchronized void generateData() {
    ...
    getDataBytes(sourceBytes);
    ByteBuffer sourceBuffer = socketService.getByteBuffer(sourceBytes);
    socketHandle.write(sourceBuffer).addDeferrable(this, true);
  }
  ...
  /*
   * Fill the specified byte array with pseudo-random data.
   */
  private void getDataBytes(byte[] target) {
    if (dataSource != null) {
      for (int i = 0; i < target.length; i++) {
        target[i] = (byte) (dataSource.nextInt());
      }
    ...
    // Log the generated data if required.
  ...
  /*
   * On callback from transmitting a previous block of data, either drain the
   * incomplete buffer or generate a new data block after a short delay.
   */
  public synchronized Void onCallback(Deferred<ByteBuffer> deferred, ByteBuffer bufferData) {
    if (bufferData == null) {
      reactor.runLater(x -> generateData(), 50 + random.nextInt(100), null);
    } else {
      socketHandle.write(bufferData).addDeferrable(this, true);
    }
    return null;
  }
  ...
  public synchronized Void onErrback(Deferred<ByteBuffer> deferred, Exception error) {
    if (error instanceof SocketClosedException) {
      logger.log(Level.INFO, "Halting test data generation on socket closed for <" + socketHandle.getSocketId() + ">",
          error);
    } else {
      logger.log(Level.SEVERE, "Test data source fatal error for <" + socketHandle.getSocketId() + ">", error);
      fatalError = error;
    }
    return null;
  }
